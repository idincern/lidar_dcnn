...
with tf.name_scope('conv_layer_0'):
    ...
    f1 = tf.nn.conv1d(f0,k0,stride=s[0],padding='VALID') + b0
...
with tf.name_scope('conv_layer_1'):
    ...
    # add Leaky ReLU and Batch Normalization
    f2 = tf.nn.conv1d(f1,k1,stride=s[1],padding='VALID') + b1
...
with tf.name_scope('conv_layer_2'):
    ...
    # add Leaky ReLU and Batch Normalization
    f3 = tf.nn.conv1d(f2,k2,stride=s[2],padding='VALID') + b2
...
with tf.name_scope('conv_layer_3'):
    ...
    # add Leaky ReLU and Batch Normalization
    f4 = tf.nn.conv1d(f3,k3,stride=s[3],padding='VALID') + b3
...
with tf.name_scope('conv_layer_4'):
    ...
    # add Leaky ReLU and Batch Normalization
    f5 = tf.nn.conv1d(f4,k4,stride=s[4],padding='VALID') + b4
...
with sess.as_default():
    ...
    output = sess.run(f8, feed_dict={f0: batch_scans})
    print(output)
    print(output.shape)
    print(h)
